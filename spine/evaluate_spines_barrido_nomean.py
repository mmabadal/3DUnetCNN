...
def main():
    ...
    parser.add_argument('--s_max', default=4500, type=int, help='max spine size')
    ...
    for index in range(0, 10, 1):  # threshold scan
        ...
        for idx, case_folder in enumerate(dir):  # for each case
            ...
            for spineGT in range(num_labels_truth):  # for each spine in gt (spineGT)
                ...
                # delete pairs that each % individually is not > cthr
                x = [coincide_list_GT[i] for i in range(len(coincide_list_GT)) if coincide_list_GT[i] >= coinc_thr]
                y = [coincide_list_Pred[i] for i in range(len(coincide_list_GT)) if coincide_list_GT[i] >= coinc_thr]
                x2 = [x[i] for i in range(len(x)) if y[i] > coinc_thr]
                y2 = [y[i] for i in range(len(y)) if y[i] > coinc_thr]

                # get maximum mean score
                coincide_list_mean = [(x + y) / 2 for x, y in zip(x2, y2)]  # scores mean
    ...
    df.to_csv(path_run + "/spine_scores_nomean.csv")
